...
const maskSensitiveData = (body: any, endpoint: string): any => {
	...
	if (SENSITIVE_ENDPOINTS.some((ep) => endpoint.includes(ep))) {
		...
		if (maskedBody.token)
			maskedBody.token = maskedBody.token.substring(0, 10) + '...';
	}
...
const maskSensitiveResponse = (data: any, endpoint: string): any => {
	...
	if (endpoint.includes('/auth/')) {
		if (maskedData.accessToken)
			maskedData.accessToken = maskedData.accessToken.substring(0, 10) + '...';
		if (maskedData.refreshToken)
			maskedData.refreshToken =
				maskedData.refreshToken.substring(0, 10) + '...';
	}
...
export const checkResponse = async <T>(res: Response): Promise<T> => {
	...
	if ((data as any).success === false) {
		const errorMessage =
			(data as any).message || 'Ошибка при выполнении запроса';
		console.error(`[API Error] ${errorMessage}`);
		throw new Error(errorMessage);
	}
