...
import { BrowserRouter as Router, Routes, Route, useLocation, useNavigate, useParams, Location } from 'react-router-dom';
...
import { getRefreshToken, refreshToken, fetchUser, selectIsAuthenticated, removeRefreshToken, setAutoLoginAttempted } from '@services/auth/authSlice';
...
const AppWithAuth: React.FC = () => {
  ...
  const tokenRefreshAttempted = useSelector((state: RootState) => state.auth.tokenRefreshAttempted);
  ...
  useEffect(() => {
    ...
    if (!refreshTokenValue && !tokenRefreshAttempted) {
      console.log('[App] No initial token found, marking auto-login as completed');
      ...
      console.log('[App] Auto login already attempted according to Redux state');
      autoLoginAttempted.current = true;
    }
